...
def display_globe(state, type='globe_3/33'):
    ...
    for i in range(rows):
        for j in range(cols):
            if len(tiles[i*cols + j]) != 1 :
                tiles[i*cols + j] = tiles[i*cols + j][1:]
...
def init_dic() :
    data = {
        # 1: [14, "r1 f13 -r0 f13 r0 f13 -r0 f13 -r1 f13 r1 r0 f13 -r1"],
        # 2: [15, "r1 f13 -r1 -r0 f13 r1 f13 r0 f13 -r0 f13 r0 f13 -r1"],
        # 3: [15, "r1 f13 -r1 -r0 f13 -r0 f13 -r1 f13 r1 f13 r0 r1 r0 f13 -r1"],
        # 4: [15, "f13 -r0 -r1 -r1 -r0 f13 r1 f13 r0 f13 -r0 f13 r0 r1 r0 f13"],
        # 5: [15, "f13 -r0 -r1 -r1 -r0 f13 -r0 f13 -r1 f13 r1 f13 r0 r1 r0 r1 r0 f13"],
        # 6: [15, "f13 -r0 -r1 -r0 -r1 -r1 f13 -r0 f13 -r1 f13 r1 f13 r1 r0 r1 r0 r1 r0 f13"],
        # 7: [15, "-r1 f13 -r1 -r0 -r1 -r0 -r1 -r0 -r1 f13 r1 f13 r0 f13 -r0 f13 r1 r0 r1 r0 r1 r0 f13 r1"],
        # 8: [15, "-r0 -r1 -r0 -r1 f13 -r0 -r1 -r0 -r1 -r0 -r1 -r0 -r1 f13 r1 f13 r0 f13 -r0 f13 r0 r1 r0 r1 r0 r1 r0 f13 r1 r0 r1 r0"],
...
num_state = ";".join([str(i) for i in range(66*4)])
for e in path[::-1]:
    if e[0] == '-' :
        e = e[1:]
    else :
...
for i in range(66*2, 66*3-2) :
    ...
    for j in range(i, 66*3) :
        if s[j] == t[i] :
            cur = j
            break
    print(cur%66, target%66)

    p = []
    while cur != target :
        ret, cur = calc_lower_pos(cur, target, rdic = dic, mod=66)
        p += ret
...
# import random
# for i in range(11):
#     cur = random.randint(10, 60)
#     off = random.randint(2, 8)
#     ret, _ = calc_lower_pos(cur, cur-off, rdic = dic, mod=66)
#     newpath += ret
#     for e in ret:
#         state = apply_move(e, state)
#         num_state = apply_move(e, num_state)

for i in range(66*3, 66*4-2) :
    target = i
    # s = list(map(int, num_state.split(";")))
    # for j in range(i, 66*4) :
    #     if s[j] == i :
    #         cur = j
    #         break
    s = state.split(";")
