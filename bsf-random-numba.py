...
# -------------------------------------------------------------------
def Search(paths:list, moves:dict, initial_state:str, solution_state:str, num_wildcards:int, pbar:tqdm, prob :float = 1.0 , last_sate:str="") :
...
if args.step < 0 :
    x = -args.step
    if x >= 100: x = random.randint(1, 10)
    step = max(10, (len(paths) + x - 1)//x)
